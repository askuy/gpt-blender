...
assert.deepEqual(await page.locator('.project-nav a').evaluateAll((links)=>links.map((link)=>link.innerText.replace(/\s+/g,' ').trim())),['001 Apple Duo','002 Pelican Ride','003 Saber','004 Niu Lai']);
assert.equal(await page.locator('a[href*=".mp4"], video').count(),0,'Homepage must not include MP4 videos or links');
assert.equal(await page.locator('html').getAttribute('lang'),'en','Homepage defaults to English');
await page.getByRole('button',{name:'Switch to Chinese'}).click();
assert.equal(await page.locator('html').getAttribute('lang'),'zh-CN','Language switch updates the document language');
assert.match(await page.locator('#hero-title').innerText(),/把想法/);
await page.getByRole('button',{name:'切换为英文'}).click();
assert.equal(await page.locator('html').getAttribute('lang'),'en','Language switch can return to English');
await page.screenshot({path:root+'renders/home-desktop.png',fullPage:true});
await page.setViewportSize({width:390,height:844});
assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,'Homepage must not overflow horizontally on mobile');
await page.screenshot({path:root+'renders/home-mobile.png',fullPage:true});
await page.setViewportSize({width:1440,height:1100});
const niulaiBase = new URL('niulai/', base).href;
await page.goto(niulaiBase);
...
await page.screenshot({path:root+'renders/niulai-orbit.png',fullPage:true});
await page.setViewportSize({width:390,height:844});
...
await page.screenshot({path:root+'renders/niulai-mobile.png',fullPage:true});
...
const result={passed:true,checks:['homepage project gallery','English default and Chinese switch','only viewer files are published','model loads','no MP4 videos or links','original audio plays after click','pause stops audio','timeline seeks','dialogue captions','actual camera orbit','replay from free camera','director reset','mobile layout','no page errors'],state};
